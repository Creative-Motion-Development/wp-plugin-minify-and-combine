...
/**
 * Class WMAC_PluginHTML
 */
class WMAC_PluginHTML extends WMAC_PluginBase
{
    /**
     * Force xhtml.
     *
    ...
    public function read( $options )
    {
        ...
        $this->forcexhtml = (bool) apply_filters( 'wmac_filter_html_forcexhtml', false );

        // Filterable strings to be excluded from HTML minification.
        $exclude = apply_filters( 'wmac_filter_html_exclude', '' );
        if ( '' !== $exclude ) {
    ...
    {
        $noptimize = apply_filters( 'wmac_filter_html_noptimize', false, $this->content );
        if ( $noptimize ) {
            return false;
        }
        ...
        if ( class_exists( 'RevSlider' ) && apply_filters( 'wmac_filter_html_dataattrib_cleanup', false ) ) {
