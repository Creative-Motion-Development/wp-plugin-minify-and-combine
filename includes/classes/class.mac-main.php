...
class WMAC_PluginMain
{
	...
	public function jsSnippetcacher( $jsin, $jsfilename )
	{
		...
		if ( $ccheck->check() ) {
			...
			if (
				false === ( strpos( $jsfilename, 'min.js' ) )
				&& ( false === strpos( $jsfilename, 'js/jquery/jquery.js' ) )
				&& ( str_replace( apply_filters( 'wmac_filter_js_consider_minified', false ), '', $jsfilename ) === $jsfilename )
			) {
			...
			if (
				! empty( $jsfilename )
				&& str_replace( apply_filters( 'wmac_filter_js_speedup_cache', false ), '', $jsfilename ) === $jsfilename
			) {
